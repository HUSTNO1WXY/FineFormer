...
class LoFTRLoss(nn.Module):
    ...
    def forward(self, data):
        ...
        # 2. fine-level loss
        # loss
        # K0 = data['K0'].squeeze().cpu()
        # K1 = data['K1'].squeeze().cpu()
        # img0, img1 = data['image0'].squeeze().cpu(), data['image1'].squeeze().cpu()
        # img0, img1 = img0.numpy(), img1.numpy()
        # kpts0, kpts1 = data['mkpts0_f'].cpu(), data['mkpts1_f'].cpu()
        # kpts0, kpts1 = kpts0.numpy(), kpts1.numpy()
        # fmatrix = data['F01'].cpu()
        # fmatrix = fmatrix.squeeze().numpy()
        # lines2 = cv2.computeCorrespondEpilines(kpts0.reshape(-1, 1, 2), 1, fmatrix)
        # lines2 = lines2.reshape(-1, 3)
        # img3, img4 = drawlines(img1, img0, lines2, kpts1, kpts0)
